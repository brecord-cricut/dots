...
hypr_style_lua="$XDG_CONFIG_HOME/hypr/style.lua"
hypr_style_lua_tmp="$XDG_RUNTIME_DIR/style.lua"
{
  echo '-- Generated by '"'"'wal'"'"' via scripts/wallpaper-post.sh'
  echo '-- Do not edit manually'
  echo ''
  echo 'return {'
  echo "    wallpaper      = \"$wallpaper\","
  echo "    background     = \"rgb(${background:1})\","
  echo "    foreground     = \"rgb(${foreground:1})\","
  echo "    cursor         = \"rgb(${cursor:1})\","
  for i in $(seq 0 15); do
    varname="color$i"
    echo "    color$i = \"rgb(${!varname:1})\","
  done
  echo "    lock_wallpaper = \"$lockscreen_file\","
  echo '}'
} > "$hypr_style_lua_tmp" && mv "$hypr_style_lua_tmp" "$hypr_style_lua"
